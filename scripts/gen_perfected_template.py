...
SECTION1 = [
    ...
    ("DATA", "支付给职工的工资", 155172.54, "来自于管理费用和销售费用中的工资", "必填", None, None,
     "全年实发工资总额（含奖金津贴）★必填，直接影响经营净额"),
    ("DATA", "支付给职工的四金", 41275.90, "来自于管理费用中和销售费用中的社保、医保费", "选填", None, None,
     "社保+公积金等（不填则按工资×26.6%自动推算）"),
    ("DATA", "支付给职工的其他福利费", 1644.83, "来自于管理费用和销售费用中的福利费", "选填", None, None,
     "其他职工福利（不填则按工资×1.06%自动推算）"),
    ("FORMULA", "合计", None, "", "自动算", None, None, "=C{r1}+C{r2}+C{r3}", "(公式：其他福利费+四金+工资)"),
    ("DATA", "销项税额", 835728.00, "", "选填", 0.06, "须根据实际税率修改", "全年销项税额（不填则按收入×6%推算）"),
    ("DATA", "进项税额", 0.00, "", "选填", 0.00, "须根据实际税率修改", "全年进项税额（不填则按成本×6%推算）"),
    ("FORMULA", "应交增值税", None, "", "自动算", None, None, "=C{r2}-C{r1}", "(公式：销项税额-进项税额)"),
    ("DATA", "其他各项税", 0.00, "", "选填", None, None, "除增值税外各税种（不填取利润表营业税金及附加）"),
    ("DATA", "所得税", 1411718.54, "", "选填", 0.25, "须根据实际税率修改", "全年所得税（不填取利润表所得税费用）"),
    ("DATA", "管理费用中列支的税金", "", "", "选填", None, None, "计入管理费用的税金（如房产税、印花税）"),
    ...
    ("DATA", "利息支出", "", "不抵减利息收入", "选填", None, None, "利息支出总额（不抵减利息收入）"),
    ...
    ("DATA", "计提坏账准备的比率", 0.005, "可根据实际计提比率修改", "选填", 0.005, "可根据实际计提比率修改",
     "坏账计提比例（默认0.5%，按公司实际改）"),
    ("DATA", "计提的坏账准备", "", "", "选填", None, None, "当年计提坏账（不填为0，与模板口径一致）"),
    ...
    ("FORMULA", "处置固定资产、无形资产和其他长期资产所收回的现金净额合计", None, "", "自动算", None, None,
